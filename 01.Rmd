...
Note that we cannot subtract inequalities the same way that we can subtract equalities. So to get started, let us assume that all the inequalities are in fact equalities. We can solve the system 
...
Here we can see that to obtain the solution $(x, y) = (0.6, 0.3)$ we're solving 
...
However, we could have switched the other inequalities to equalities instead and obtained a different solution. We'd then need to compare the yield in the solutions obtained in each of these cases and find one that maximizes it. This method becomes unwieldy very fast and we'll need to develop better ways to solve linear programs. 
</details>