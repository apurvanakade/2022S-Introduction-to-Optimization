...
The **range of optimality** for $b_i$ is the interval $(b_i + \delta_-, b_i + \delta_+)$ such that $\mathcal{B}^{-1} b + \delta (\mathcal{B}^{-1})_{*i} \ge 0$ for all $\delta \in (\delta_-, \delta_+)$.
...
\begin{align}
  \dfrac{\partial x_{\mathcal{B}_j}}{\partial b_i} 
    ...
    &= (\mathcal{B}^{-1})_{ji} \\
\end{align}
where $x_{\mathcal{B}_j}$ denotes the $j^{th}$ basic variable and 
...
\implies
  \dfrac{\partial \mathbb{O}}{\partial b_i} 
    ...
    &= c^T_{\mathcal{B}} (\mathcal{B}^{-1})_{*i} \\
\end{align}
...
\begin{align}
  y^*_i = c^T_{\mathcal{B}} (\mathcal{B}^{-1})_{*i}.
\end{align}
