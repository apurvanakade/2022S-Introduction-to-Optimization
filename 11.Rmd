...
\begin{align}
  ...
  \begin{bmatrix} w_1 \\ w_2 \end{bmatrix} \\
  & 
  = \begin{bmatrix} 0.6 \\ 0.3 \\ 0.9 \end{bmatrix} 
  -
  \begin{bmatrix}
    -1/9 & 2/3 \\ 
    2/9 & -1/3 \\
...
So, $\delta_- = -1.35$ and $\delta_+ = \min(5.4, 0.9) = 0.9$ and the range of optimality of $b_1$ is $(3.6 - 1.35, 3.6 + 0.9) = (2.85, 3.45)$.
...
So, $\delta_- = -0.9$ and $\delta_+ = \min(0.3, 0.9) = 0.3$ and the range of optimality of $b_2$ is $(1.5 - 0.9, 1.5 + 0.3) = (0.6, 1.8)$.
...
which gives us $\delta \ge -0.1$ and so the range of optimality of $b_3$ is $(1 - 0.1, \infty) = (0.9, \infty)$.
...
\begin{align}
  \dfrac{\partial x_i}{\partial b_i} 
    &= j^{th} \mbox{ row of } \dfrac{\partial \mathcal{B}^{-1}b}{\partial b_i} \\
    &= (\mathcal{B}^{-1})_{ji} \\
  \implies 
  \dfrac{\partial \mathbb{O}}{\partial b_i} 
    &= \dfrac{\partial c^T x}{\partial b_i} \\
    &= \sum \limits_{j=1}^n c_j (\mathcal{B}^{-1})_{ji} \\
    &= c^T (\mathcal{B}^{-1})_{*i} \\
\end{align}

::: {.theorem #rate-of-change-constraints}

For a non-degenerate linear program, the rate of change of the optimal solution with respect to $b_i$ is given by $c^T (\mathcal{B}^{-1})_{*i}$.
...
There is an alternate way to find the rate of change of the objective function with respect to $b_i$. By strong duality, we know that the optimal objective value of the primal and dual are the same so that at the optimal solution $\mathbb{O} = b^T y^*$ where $y^*$ is a dual optimal solution. Then 

\begin{align}
  \dfrac{\partial \mathbb{O}}{\partial b_i} = y^*_i.
\end{align}

Because of this relationship, the dual optimal solution is also called the **marginal cost** or the **shadow price** of $b_i$.
Combining this result with \@ref(thm:rate-of-change-constraints) we get 

::: {.theorem #dual-optimal-values}

For a non-degenerate linear program, the dual optimal solution is given by $c^T \mathcal{B}^{-1}$.

:::

This gives us a way to compute the dual optimal solution directly without having to run the simplex method again on the dual linear program.