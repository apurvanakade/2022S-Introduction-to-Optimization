...
Because the domain of $\ln(x)$ is $(0, \infty)$, any critical point of $f_\mu(x)$ must lie in the feasible region of \@ref(eq:constrained-optimization-problem). Moreover, if the following unconstrained optimization problem has an optimal solution then it will be away from the boundary of \@ref(eq:constrained-optimization-problem) as the *cost* of approaching the boundary goes to $\infty$:

\begin{align}
  \mbox{minimize: } & f_\mu (x).
\end{align}

Let $x_\mu^*$ be a solution of the above problem. We'll make the following assumption without proof: *$x_\mu^*$ exists and is continuous in $\mu$ and $\lim \limits_{\mu \to 0^+} x_\mu^* = x^*$ where $x^*$ is an optimal solution to \@ref(eq:constrained-optimization-problem).* This assumption is valid, for example, when $f(x)$ and $g_i(x)$ are convex functions and the feasible region has an interior. With this assumption, we now have a method for solving \@ref(eq:constrained-optimization-problem):
