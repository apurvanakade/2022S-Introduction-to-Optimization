...
\end{equation}
where $-\mathcal{B}_d$ is the basic submatrix of augemented dual $\begin{bmatrix} -A^T & I_n\end{bmatrix}$ and so $\mathcal{B}_d$ is the basic submatrix of augemented dual $\begin{bmatrix} A^T & -I_n\end{bmatrix}$. This simplifies to 
\begin{equation}
  \mathcal{B}_d^{-1} c - \delta (\mathcal{B}_d^{-1})_{\_j} \ge 0.
\end{equation}
...
where $\mathcal{B}$ is formed by combining the primal-basic columns in $\begin{bmatrix} A & I_m \end{bmatrix}$, and 
...
where $\mathcal{B}_d$ is formed by combining the dual-basic columns in $\begin{bmatrix} A^T & -I_n \end{bmatrix}$. 
:::
